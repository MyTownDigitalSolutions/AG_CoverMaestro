...
export default function MaterialRoleConfigsPage() {
    ...
    const handleSaveNew = async () => {
        ...
        try {
            ...
            await axios.post('/api/material-role-configs', payload)
            setNewRow(null)
            setError(null)
            loadData()
        } catch (err: any) {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            await axios.delete(`/api/material-role-configs/${id}`)
            setError(null)
            loadData()
        } catch (err: any) {
                    ...
                    <TableBody>
                        ...
                        {configs.map((config) => (
                            <TableRow key={config.id}>
                                <TableCell>
                                    <Typography variant="body2" fontWeight="medium">{config.role}</Typography>
                                </TableCell>
                                <TableCell>
                                    <TextField
                                        size="small"
